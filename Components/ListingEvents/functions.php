...
add_filter('Flynt/addComponentData?name=ListingEvents', function ($data) {
    ...
    // Convert Timber\PostQuery to array for sorting
    $postsArray = iterator_to_array($posts);
    
    // Sort posts so recurring events appear first
    usort($postsArray, function($a, $b) {
        $aRecurring = get_field('is_recurring', $a->ID) ? 1 : 0;
        $bRecurring = get_field('is_recurring', $b->ID) ? 1 : 0;
        
        // If both are recurring or both are not, maintain original order (by end_date)
        if ($aRecurring === $bRecurring) {
            return 0;
        }
        
        // Recurring events come first
        return $bRecurring - $aRecurring;
    });
